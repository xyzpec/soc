...
bool sl_power_manager_is_ok_to_sleep(void)
{
  ...
  if (throughput_central_is_ok_to_sleep() == false) {
    ok_to_sleep = false;
  }
  if (throughput_peripheral_is_ok_to_sleep() == false) {
    ok_to_sleep = false;
  }
  // Application hook
...
bool sl_power_manager_sleep_on_isr_exit(void)
{
  ...
  answer = throughput_central_sleep_on_isr_exit();
  if (answer == SL_POWER_MANAGER_WAKEUP) {
    force_wakeup = true;
  } else if (answer == SL_POWER_MANAGER_SLEEP) {
    sleep = true;
  ...
  answer = throughput_peripheral_sleep_on_isr_exit();
  if (answer == SL_POWER_MANAGER_WAKEUP) {
    force_wakeup = true;
  } else if (answer == SL_POWER_MANAGER_SLEEP) {
    sleep = true;
